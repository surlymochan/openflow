import { createIntervention } from '../../mission-state.js';
import { mutateStore, resolveDataFile } from '../../state-store.js';
import { resolveBoundMissionId } from '../../bindings.js';

export async function interventionCreate(input = {}, context = {}) {
  ...
  if (!missionId) {
    throw new Error('mission_id is required for D5.intervention.create');
  }

  const result = await mutateStore(
    (state) => createIntervention(state, missionId, input),
    dataFile,
  );
